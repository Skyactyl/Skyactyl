...
class UpdateChecker extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            $response = Http::get('https://skyactyl.dev/api/v2/get-version');
            if ($response->failed()) {
            ...
            if (!$latestVersion) {
                $this->error('Server sent invalid response. Reach out to Skyactyl support team.');
            ...
            if ($latestVersion === $currentVersion) {
                ...
                $this->info('You\'re running pre-release of Skyactyl Panel.');
            }
        } catch (\Exception $e) {
