...
class SoftwareVersionService
{
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://skyactyl.dev/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('skyactyl.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('skyactyl.cdn.url'));
